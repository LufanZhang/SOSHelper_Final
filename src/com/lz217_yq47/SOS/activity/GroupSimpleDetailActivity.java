			...
			public void run() {
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							progressBar.setVisibility(View.INVISIBLE);
							Toast.makeText(GroupSimpleDetailActivity.this, "Fail to get history records: "+e.getMessage(), 1).show();
						}
					});
				}
			...
			public void run() {
				try {
					if(group.isMembersOnly()){
						EMGroupManager.getInstance().applyJoinToGroup(groupid, "Request add");
					}else{
					...
					runOnUiThread(new Runnable() {
						public void run() {
							pd.dismiss();
							if(group.isMembersOnly())
								Toast.makeText(GroupSimpleDetailActivity.this, "Request send success, waiting for response", 0).show();
							else
								Toast.makeText(GroupSimpleDetailActivity.this, "Successful", 0).show();
					...
					runOnUiThread(new Runnable() {
						public void run() {
							pd.dismiss();
							Toast.makeText(GroupSimpleDetailActivity.this, "Failure："+e.getMessage(), 0).show();
						}
					});
				}
			}
