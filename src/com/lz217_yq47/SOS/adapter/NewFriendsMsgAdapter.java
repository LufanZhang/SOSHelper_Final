	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (msg != null) {
			...
			if (msg.getStatus() == InviteMesageStatus.BEAGREED) {
				...
				holder.reason.setText("Your follow application has been accepted");
				...
				if(msg.getStatus() == InviteMesageStatus.BEINVITEED){
					if (msg.getReason() == null) {
						// 如果没写理由
						holder.reason.setText("Request to follow you");
					}
				}else{ //入群申请
					if (TextUtils.isEmpty(msg.getReason())) {
						holder.reason.setText("Request to join the group chat：" + msg.getGroupName());
					}
				}
				// 设置点击事件
			...
			public void run() {
				...
				try {
					...
					((Activity) context).runOnUiThread(new Runnable() {
						...
						public void run() {
							pd.dismiss();
							Toast.makeText(context, "Acception fail: " + e.getMessage(), 1).show();
						}
					});
